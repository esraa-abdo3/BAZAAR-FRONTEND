...
//   const logoUrl     = bazaar?.backgroundImage;
...
{!priceData && (
  <div className="text-2xl font-extrabold text-green-300 mb-5">
    Free Entry 🎉
      ...
      {/* Share & QR Code */}
      <section className="mt-14">
        <div className="rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-center gap-8"
          style={{ background: "#f9fafb", border: "1px solid #e5e7eb" }}>

          <div className="flex-shrink-0 bg-white p-3 rounded-2xl" style={{ border: "1px solid #e5e7eb" }}>
            {shareUrl && (
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(shareUrl)}`}
                alt="QR code to this bazaar page"
                width={160}
                height={160}
                className="w-40 h-40"
              />
          ...
          <div className="flex-1 w-full text-center sm:text-left">
            <h2 className="font-extrabold text-gray-900 text-xl mb-1">Share This Bazaar</h2>
            <p className="text-gray-500 text-sm mb-4">
              Scan the QR code or copy the link below to share this bazaar with your friends.
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-stretch gap-3">
              <input
                ...
                className="flex-1 px-4 py-3 rounded-xl text-sm text-gray-600 bg-white truncate"
                style={{ border: "1px solid #e5e7eb" }}
              />
              <button
                onClick={handleCopyLink}
                className="px-5 py-3 rounded-xl text-sm font-bold text-white transition hover:brightness-110 cursor-pointer whitespace-nowrap"
                style={{ background: "#50604A" }}
              >
