...
export default function BrandSettings() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res  = await axios.get(`${BASE_URL}/brand/profile`, { headers: getHeaders() });
  ...
  async function handleSave() {
    ...
    try {
      ...
      await axios.put(`${BASE_URL}/brand/profile`, fd, { headers: getHeaders() });
      setSuccess(true);
    } catch (err) {
  ...
  const inputClass = "w-full border border-stone-200 rounded-lg px-3 py-2.5 text-sm text-stone-800 bg-white focus:outline-none focus:border-[#3d4f38] transition-colors placeholder:text-stone-300";
    ...
    <div className="max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-xs text-stone-400 mt-1">Curate your brand&apos;s digital presence and core identity.</p>
        </div>
        <div className="flex gap-2">
          ...
          <button
            onClick={() => setForm({ brandName:"", description:"", phone:"", whatsapp:"", email:"", address:"", logo:null })}
      ...
      {error && (
        <div className="mb-5 bg-red-50 border border-red-200 text-red-600 text-xs px-4 py-3 rounded-xl">{error}</div>
      )}

      {/* Brand Identity */}
      <div className="bg-white rounded-xl border border-stone-200 p-6 mb-4">
        <h2 className="text-sm font-semibold text-stone-800 mb-5">Brand Identity</h2>
        ...
        <div className="flex items-start gap-5 mb-5">
          ...
          <div className="flex-1 flex flex-col gap-4">
            <div>
              ...
              <input name="brandName" value={form.brandName} onChange={handleChange} placeholder="Digital Atelier" className={inputClass} />
            </div>
            <div>
      ...
      {/* Contact Info */}
      <div className="bg-white rounded-xl border border-stone-200 p-6 mb-4">
        <h2 className="text-sm font-semibold text-stone-800 mb-5">Contact Info</h2>
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            <label className={labelClass}>Phone Number</label>
            <div className="relative">
              <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-400" width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <input name="phone" value={form.phone} onChange={handleChange} placeholder="+1 (555) 012-3456" className={`${inputClass} pl-9`} />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input name="whatsapp" value={form.whatsapp} onChange={handleChange} placeholder="+1 (555) 098-7654" className={`${inputClass} pl-9`} />
            </div>
          </div>
        </div>
        <div>
          ...
          <div className="relative">
            ...
            <input name="email" type="email" value={form.email} onChange={handleChange} placeholder="concierge@digitalatelier.com" className={`${inputClass} pl-9`} />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-6">
        <h2 className="text-sm font-semibold text-stone-800 mb-5">Location Details</h2>
        <div>
          <label className={labelClass}>Current Address</label>
          <div className="relative">
            <svg className="absolute left-3 top-3 text-stone-400" width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
