...
function StockBadge({ stock }) {
  if (stock === 0)   return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-600 uppercase">Out</span>;
  if (stock <= 5)    return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-orange-100 text-orange-600 uppercase">Low Stock</span>;
  return <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-green-100 text-green-600 uppercase">In Stock</span>;
}
...
export default function BrandProducts({ onAddNew, onEdit }) {
  ...
  async function load() {
    try {
      const res  = await axios.get(`${BASE_URL}/products/brand`, { headers: getHeaders() });
  ...
  async function handleDelete(id) {
    ...
    setDeleting(id);
    try {
      await axios.delete(`${BASE_URL}/products/${id}`, { headers: getHeaders() });
  ...
  async function adjustStock(id, delta) {
    ...
    if (!product) return;
    const newStock = Math.max(0, (product.stock ?? product.quantity ?? 0) + delta);
    try {
      await axios.put(`${BASE_URL}/products/${id}`, { stock: newStock }, { headers: getHeaders() });
      setProducts((prev) => prev.map((p) => p._id === id ? { ...p, stock: newStock } : p));
    } catch {/* ignore */}
  }

  const active   = products.filter((p) => (p.stock ?? p.quantity ?? 0) > 5).length;
  const lowStock = products.filter((p) => { const s = p.stock ?? p.quantity ?? 0; return s > 0 && s <= 5; }).length;
  const outStock = products.filter((p) => (p.stock ?? p.quantity ?? 0) === 0).length;
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          { label: "Active", value: active, icon: "✓", color: "text-green-600 bg-green-50" },
          { label: "Low Stock Items", value: lowStock, icon: "⚠", color: "text-orange-600 bg-orange-50" },
          { label: "Out of Stock Items", value: outStock, icon: "✕", color: "text-red-600 bg-red-50" },
        ].map((s) => (
          <div key={s.label} className="bg-white rounded-xl border border-stone-200 p-4 flex items-center gap-3">
            <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${s.color}`}>{s.icon}</span>
            <div>
      ...
      {lowStock > 0 && (
        <div className="bg-white rounded-xl border border-orange-200 p-4 mb-5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs font-semibold text-stone-700">Priority Restock Needed</p>
          </div>
          <p className="text-xs text-stone-500 flex-1 mx-4 hidden md:block">
            The following items are running low and will likely sell out within 48 hours.
          </p>
      ...
      <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
        <div className="px-5 py-4 border-b border-stone-100">
          <h2 className="text-sm font-semibold text-stone-800">Inventory Catalog</h2>
        </div>
        ...
        {loading ? (
          ...
          <div className="py-16 text-center">
            ...
            <button onClick={onAddNew} className="text-xs bg-[#3d4f38] text-white px-5 py-2 rounded-lg">Add Product</button>
          </div>
        ) : (
          <>
            <table className="w-full text-xs">
              ...
              <tbody>
                {products.map((product, i) => {
                  ...
                  return (
                    <tr key={product._id ?? i} className="border-b border-stone-50 last:border-0 hover:bg-stone-50/40 transition-colors">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-2.5">
                          {product.images?.[0] ? (
                            <img src={product.images[0]} alt={product.name} className="w-9 h-9 rounded-lg object-cover border border-stone-100 shrink-0" />
                          ) : (
                            <div className="w-9 h-9 rounded-lg bg-stone-100 shrink-0" />
                          )}
                          <div>
                            <p className="font-semibold text-stone-800">{product.name}</p>
                            <p className="text-[10px] text-stone-400">{product.category ?? "No category"}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3 text-stone-500 font-mono">
                        {product.sku ?? `SKU-${(product._id ?? String(i)).slice(-4).toUpperCase()}`}
                      </td>
                      <td className="px-5 py-3 text-stone-600">{product.demand ?? product.sold ?? 0}</td>
                      ...
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="w-8 text-center font-semibold text-stone-800">{stock}</span>
                          <button
                            onClick={() => adjustStock(product._id, +1)}
                            className="w-5 h-5 rounded border border-stone-200 flex items-center justify-center text-stone-500 hover:bg-stone-100 transition-colors font-bold"
                          >
