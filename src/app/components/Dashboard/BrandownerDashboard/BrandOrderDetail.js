...
export default function BrandOrderDetail({ order, onBack }) {
  ...
  const subtotal = Number(order.totalPrice ?? order.total ?? 0);
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <p className="text-[10px] uppercase tracking-widest text-stone-400 mb-1">Transaction Receipt</p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div className="xl:col-span-2 flex flex-col gap-5">
          ...
          <div className="bg-white rounded-xl border border-stone-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-stone-800">The Manifest</h2>
              <span className="text-xs text-stone-400">{items.length} items</span>
            </div>
            {items.length === 0 ? (
              <p className="text-sm text-stone-400">No item details available</p>
            ) : (
              <div className="flex flex-col gap-4">
                {items.map((item, i) => (
                  <div key={i} className="flex items-center gap-4">
                    {item.product?.images?.[0] ? (
                      <img src={item.product.images[0]} alt="" className="w-16 h-16 rounded-lg object-cover border border-stone-100 shrink-0" />
                    ) : (
                      <div className="w-16 h-16 rounded-lg bg-stone-100 shrink-0" />
                    )}
                    <div className="flex-1">
                      <p className="text-sm font-semibold text-stone-800">{item.product?.name ?? item.name ?? "Product"}</p>
                      {item.variant && <p className="text-xs text-stone-400 mt-0.5">{item.variant}</p>}
                      <p className="text-xs text-stone-400">SKU: {item.product?.sku ?? "—"}  ·  QTY: {item.quantity ?? 1}</p>
                    </div>
                    <p className="text-sm font-semibold text-stone-800 shrink-0">
                      ${Number((item.product?.price ?? item.price ?? 0) * (item.quantity ?? 1)).toFixed(2)}
                    </p>
                  </div>
                ))}
              </div>
          ...
          {/* Financial Summary */}
          <div className="bg-white rounded-xl border border-stone-200 p-6">
            <h2 className="text-sm font-semibold text-stone-800 mb-4">Financial Summary</h2>
            <div className="flex flex-col gap-2 text-sm">
              <div className="flex justify-between text-stone-600">
                <span>Subtotal</span><span>${subtotal.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-stone-600">
                <span>Express Shipping</span><span>${shipping.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-stone-600">
              ...
              <div className="flex justify-between font-bold text-stone-900 text-base">
                <span>Total Amount</span><span>${totalAmt.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white rounded-xl border border-stone-200 p-5">
            <h3 className="text-[10px] font-semibold uppercase tracking-widest text-stone-400 mb-4">Customer Intelligence</h3>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-sm font-semibold text-stone-800">{customer}</p>
                <p className="text-[10px] text-stone-400 uppercase tracking-wide">Loyalty: Tier One</p>
              </div>
            </div>
            <div className="flex flex-col gap-2 text-xs text-stone-600">
              ...
              <div className="flex items-start gap-2">
                <svg width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="#9ca3af" strokeWidth={2} className="mt-0.5 shrink-0">
          ...
          {/* Order History */}
          <div className="bg-white rounded-xl border border-stone-200 p-5">
            <h3 className="text-[10px] font-semibold uppercase tracking-widest text-stone-400 mb-4">Order History</h3>
            <div className="relative pl-4">
              <div className="absolute left-1.5 top-0 bottom-0 w-px bg-stone-200" />
              {[
                { label: "Processing", sub: "Received, preparing..." },
                { label: "Payment Confirmed", sub: "Via AMEX Platinum", done: isPaid },
                { label: "Order Placed", sub: date, done: true },
              ].map((step, i) => (
                <div key={i} className="relative mb-4 last:mb-0">
                  <div className={`absolute -left-2.5 top-0.5 w-2 h-2 rounded-full border-2 ${step.done ? "bg-[#3d4f38] border-[#3d4f38]" : "bg-white border-stone-300"}`} />
                  <p className="text-xs font-medium text-stone-800">{step.label}</p>
          ...
          {/* Update Status */}
          <div className="bg-white rounded-xl border border-stone-200 p-5">
            <h3 className="text-[10px] font-semibold uppercase tracking-widest text-stone-400 mb-3">Update Status</h3>
