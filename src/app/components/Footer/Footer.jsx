...
export default function Footer() {
  ...
  return (
    <footer style={{ backgroundColor: "#2a3426" , width:"100%", padding:"0", margin:"0" }} className="text-white w-full">
      ...
      <div className=" py-8 px-6">
        <div className="w-full mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
          <div>
            ...
            <p className="text-white/60 text-sm">Get the latest bazaars and drops straight to your inbox.</p>
          </div>
          {subscribed ? (
      ...
      <div className="max-w-6xl mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-4 gap-10">
        ...
        <div className="md:col-span-1">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center">
              <span className="text-[#50604a] font-black text-sm">R</span>
            </div>
            <span className="text-xl font-bold tracking-wide">BAZARRNA</span>
          </div>
          <p className="text-white/60 text-sm leading-relaxed">
            The first bazaar marketplace — connecting sellers and buyers in a real market experience, online and on-ground.
          </p>
        </div>

        <div>
          <h4 className="font-semibold mb-4 text-white/90 text-sm uppercase tracking-wider">Quick Links</h4>
          <ul className="space-y-2 text-sm text-white/60">
            {["Home", "Upcoming Bazaars", "Join as a Seller", "About Us"].map((link) => (
              <li key={link}>
        ...
        <div>
          <h4 className="font-semibold mb-4 text-white/90 text-sm uppercase tracking-wider">Support</h4>
          <ul className="space-y-2 text-sm text-white/60">
            {["Contact Us", "Privacy Policy", "Terms & Conditions", "FAQ"].map((link) => (
              <li key={link}>
        ...
        <div>
          <h4 className="font-semibold mb-4 text-white/90 text-sm uppercase tracking-wider">Follow Us</h4>
      ...
      <div className="border-t border-white/10 py-4 px-6 text-center text-white/40 text-xs">
        © {new Date().getFullYear()} RooteX. All rights reserved.
      </div>
    </footer>
  );
}
