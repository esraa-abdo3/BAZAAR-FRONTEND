...
const stripePromise = loadStripe(process.env.STRIPE_PUBLISH_KEY || "pk_test_XXXXXXXX");
...
export default function Bazaradd() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      formData.append("priceOffline", String(Number(form.priceOffline)));
      formData.append("priceOnline", String(Number(form.priceOnline)));
      formData.append("priceHybrid", String(Number(form.priceHybrid)));
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 mt-15">
        ...
        {success ? (
          <div className="text-center py-20">
            <div className="text-5xl mb-4">🎉</div>
            <h2 className="text-2xl font-bold text-[#2c3020] mb-2">Bazaar Registered Successfully!</h2>
            <p className="text-gray-600 mb-2">Your bazaar is now active. Check your email — we've sent you your password so you can log in.</p>
            <p className="text-sm text-[#9A5F4C] font-medium">Please check your email and try logging in again.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
